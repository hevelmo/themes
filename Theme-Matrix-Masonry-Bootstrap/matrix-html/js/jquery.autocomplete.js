...
var product_url = 'product.php?product_id=';
...
jQuery.fn.autocomplete = function(url, settings ) 
{
	return this.each( function()//do it for each matched element
	{
		...
		valueInput.before('<ul class="autocomplete"></ul>');
		var list = valueInput.prev().css({top: textInput.offset().top + textInput.outerHeight() - 29,/* left: textInput.offset().left,*/ width: 195/*textInput.width() + 50*/});
		var oldText = '';
		var typingTimeout;
		var size = 0;
		var selected = 0;
				...
				$.getJSON(url,settings.parameters,function(data)
				{
					...
					if (data)
					{
						...
						for (i = 0; i < data.length; i++)//iterate over all options
						{
						  ...
						  list.addClass("show").children().
						  hover(function() { $(this).addClass("selected").siblings().removeClass("selected");}, function() { $(this).removeClass("selected") } ).
						  click(function () { valueInput.val( $(this).attr('value') );textInput.val( $(this).text() ); clear(); loadUrl(product_url + $(this).attr('value'), true);});
