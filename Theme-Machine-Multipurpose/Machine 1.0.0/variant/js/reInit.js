function reInit(target){
	...
	$(target+' .foreground-image-holder').each(function(){
		var imgSrc= $(this).children('img').attr('src');
		$(this).css('background', 'url("' + imgSrc + '")');
    	$(this).children('img').hide();
        $(this).css('background-position', '50% 50%');
	...
	if($(target+' nav').hasClass('relative-nav')){
		$(target+' .nav-container').css('height', $(target+' nav').outerHeight());
	}
		
	// Trigger menu toggle on nav-2
	
	$(target+' .nav-2 .menu-toggle').click(function(){
		$(this).toggleClass('form-cross');
		$(this).closest('nav').find('.menu').toggleClass('expand');
	});
	
	$(target+' .nav-3 .menu-toggle').click(function(){
		$(this).toggleClass('form-cross');
		$(this).closest('nav').find('.fullscreen-container').toggleClass('expand');
		setTimeout(function(){
			$(target+' .fullscreen-container .vertical-align').toggleClass('show-menu');
		}, 300);
	});
	...
	$(target+' .search-bar i').click(function(){
	...
	$(target+' .expanding-list .title').click(function(){
		$(this).closest('li').toggleClass('active');
	});
	
	// Quantity control for products
	
	$(target+' .add-to-cart .less').click(function(){
	...
	$(target+' .add-to-cart .more').click(function(){
		var newVal = parseInt($(this).closest('form').find('input[type="text"]').val()) + 1;
		$(this).closest('form').find('input[type="text"]').val(newVal);
	});
    ...
    if (!(/Android|iPhone|iPad|iPod|BlackBerry|Windows Phone/i).test(navigator.userAgent || navigator.vendor || window.opera)) {
		...
		$(window).scroll(function(){
			if($(window).scrollTop() < $(target+' .fixed-header').outerHeight()){
				...
				$(target+' .fixed-header').css({transform: 'translateY('+scroll/1.2+'px)'});
				$(target+' .fixed-header .container').css('opacity',(1-(scroll/400)));
			}
		});
    ...
    $(target+' .promo-1 .btn').mouseenter(function(){
    ...
    $(target+' .hover-content').mouseenter(function(){
    ...
    // Populate filters
    $(target+' .contained-projects').each(function(){
    ...
    $(target+' .video-strip .pre-video i').click(function(){
    ...
    $(target+' .video-strip .close-frame').click(function(){
    ...
    $(target+' .hero-slider').flexslider({ directionNav: false });
    $(target+' .testimonials').flexslider({ directionNav: false });
    $(target+' .image-slider').flexslider({ directionNav: false });
   ...
   // Twitter Feed
    ...
    $(target+' .planner-option').click(function(){
    ...
    $(target+' .planner-option,.planner-radio').each(function(){
    	var value = $(this).find('span').text();
    	$(this).find('input').attr('value', value);
    });
    
    $(target+' .mock-radio').click(function(){
    ...
    $(target+' form.email-form, form.project-planner').submit(function (e) {
	...
	$(window).scroll(function(){
		$(target+' .map-holder').removeClass('disable-overlay');
	});
    ...
    $(target+' .background-image-holder').addClass('fadeIn');
    $(target+' .foreground-image-holder').addClass('fadeIn');
